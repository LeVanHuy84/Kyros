...
import type { Conversation } from '../../types/memory';
...
export const ConversationsDirectoryPanel: React.FC<ConversationsDirectoryPanelProps> = ({
  onSuccess,
  onError,
  onViewTurns,
}) => {
  ...
  const handleClear = async (id: string) => {
    if (window.confirm('Clear all chat messages in this conversation thread?')) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', position: 'relative' }}>
      ...
      <div>
        <h4 style={{ margin: 0, fontSize: '18px', fontWeight: '600' }}>Conversations Directory</h4>
        <p style={{ fontSize: '15px', color: 'var(--text-muted)', margin: '4px 0 0 0', lineHeight: '1.6' }}>
          Archive of past conversation threads and commands parsed by the LLM planner. Clear logs to clean agent context.
        </p>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '15px' }}>
          <thead>
            <tr style={{ backgroundColor: 'var(--bg-app)', borderBottom: '1px solid var(--border-color)', color: 'var(--text-muted)' }}>
              <th style={{ padding: '16px 20px', fontWeight: '600' }}>Chat Thread Title</th>
              <th style={{ padding: '16px 20px', fontWeight: '600', width: '120px' }}>Status</th>
              <th style={{ padding: '16px 20px', fontWeight: '600', width: '220px' }}>Last Active</th>
              <th style={{ padding: '16px 20px', fontWeight: '600', width: '240px', textAlign: 'right' }}>Actions</th>
            </tr>
          </thead>
          <tbody>
              ...
              conversations.map((conv) => (
                <tr key={conv.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding: '16px 20px' }}>
                    <span
                      style={{
                        ...
                        fontSize: '12px',
                        fontWeight: '600',
                        backgroundColor: conv.status === 'Active' ? '#E1F5FE' : conv.status === 'Cleared' ? '#ECEFF1' : '#FBE9E7',
                        color: conv.status === 'Active' ? '#0288D1' : conv.status === 'Cleared' ? '#546E7A' : '#D84315',
                      }}
                    >
                      {conv.status}
                    </span>
                  ...
                  <td style={{ padding: '16px 20px', color: 'var(--text-muted)', fontSize: '14px' }}>
                    {conv.lastTurnTimestamp ? new Date(conv.lastTurnTimestamp).toLocaleString() : 'N/A'}
                  </td>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          padding: '6px 12px',
                          borderRadius: '4px',
                          cursor: conv.status === 'Cleared' ? 'default' : 'pointer',
