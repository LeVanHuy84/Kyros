...
const TaskManagement: React.FC = () => {
  ...
  const [autoScheduleSuccessMsg, setAutoScheduleSuccessMsg] = useState<string | null>(null);
  ...
  const fetchCalendarEvents = useCallback(async () => {
    ...
    try {
      ...
      const end = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate() + 14);
  ...
  const handleAutoSchedule = async () => {
    ...
    try {
      ...
      setAutoScheduleSuccessMsg(`Successfully scheduled ${scheduledList.length} task(s) into your calendar!`);
      fetchCalendarEvents();
      fetchTasks();
    } catch (err: any) {
  ...
  const handleSaveEvent = async (
    ...
    if (isEditing && selectedEvent) {
      ...
      if (
        new Date(selectedEvent.startTime).getTime() !== new Date(start).getTime() ||
    ...
    return (
      <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
        ...
        <p style={{ fontSize: '15px' }}>
          Please select or create a workspace from the sidebar to access Time-Blocking Workspace.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '14px', margin: 0 }}>
            Organize tasks on the left, auto-schedule or drag & drop onto your interactive calendar on the right.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              ...
              boxShadow: '0 4px 12px rgba(var(--color-primary-h), var(--color-primary-s), var(--color-primary-l), 0.25)',
              gap: '8px',
            }}
          >
            <Sparkles size={18} className={isAutoScheduling ? 'spin' : ''} />
            <span>{isAutoScheduling ? 'Auto-Scheduling...' : '🤖 AI Auto-Schedule'}</span>
          </button>
      ...
      >
        ...
        <div className="card" style={{ padding: '20px', gap: '20px', display: 'flex', flexDirection: 'column' }}>
          {/* Tab Switcher */}
          <div
            style={{
              display: 'flex',
          ...
          {isTasksLoading ? (
            ...
            <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--text-muted)' }}>
              <ListTodo size={36} style={{ opacity: 0.3, marginBottom: '8px' }} />
              <div style={{ fontWeight: '600', fontSize: '14px', color: 'var(--text-main)' }}>No Tasks Available</div>
              <div style={{ fontSize: '13px' }}>Create a task or trigger AI Auto-Schedule.</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '680px', overflowY: 'auto' }}>
              {tasks.map((task) => (
                <div
                  ...
                  style={{
                    padding: '14px 16px',
                    backgroundColor: task.status === 'Completed' ? 'rgba(16, 185, 129, 0.03)' : 'var(--bg-app)',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                    {/* Drag Handle */}
                    <span style={{ color: 'var(--text-muted)', cursor: 'grab', marginTop: '2px', opacity: 0.6 }} title="Drag to schedule on calendar">
                      ⋮⋮
                    </span>
                        ...
                        {task.autoSchedule && (
                          <span
                            style={{
                              ...
                              padding: '1px 6px',
                              borderRadius: '4px',
                              backgroundColor: 'rgba(var(--color-primary-h), var(--color-primary-s), var(--color-primary-l), 0.1)',
                              color: 'var(--color-primary)',
                              fontWeight: '600',
                            }}
        ...
        <div className="card" style={{ padding: '20px', gap: '16px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '15px', fontWeight: '700', color: 'var(--text-main)', marginLeft: '8px' }}>
              ...
              {(['week', 'day'] as const).map((m) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: viewMode === m ? 'var(--bg-card)' : 'transparent',
      ...
      <EventDetailsDrawer
        ...
        onReschedule={async (id, s, e) => {
          await apiClient.post(`/v1/workspaces/${activeWorkspace.id}/calendar/events/${id}/reschedule`, {
